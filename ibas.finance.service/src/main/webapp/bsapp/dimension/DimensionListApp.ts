...
namespace finance {
    export namespace app {
        /** 列表应用-维度 */
        export class DimensionListApp extends ibas.BOListApplication<IDimensionListView, bo.Dimension> {
            /** 应用标识 */
            static APPLICATION_ID: string = "7670a9ce-757d-4404-bf17-c87f3ec7702f";
            /** 应用名称 */
            static APPLICATION_NAME: string = "finance_app_dimension_list";
            /** 业务对象编码 */
            static BUSINESS_OBJECT_CODE: string = bo.Dimension.BUSINESS_OBJECT_CODE;
            /** 构造函数 */
            constructor() {
                super();
                this.id = DimensionListApp.APPLICATION_ID;
                this.name = DimensionListApp.APPLICATION_NAME;
                this.boCode = DimensionListApp.BUSINESS_OBJECT_CODE;
            ...
            protected deleteData(data: bo.Dimension | bo.Dimension[]): void {
                ...
                let beDeleteds: ibas.ArrayList<bo.Dimension> = new ibas.ArrayList<bo.Dimension>();
                ...
                this.messages({
                    ...
                    onCompleted(action: ibas.emMessageAction): void {
                        if (action === ibas.emMessageAction.YES) {
                            try {
                                let boRepository: bo.BORepositoryFinance = new bo.BORepositoryFinance();
                                let saveMethod: Function = function (beSaved: bo.Dimension): void {
        ...
        /** 视图-维度 */
        export interface IDimensionListView extends ibas.IBOListView {
            /** 编辑数据事件，参数：编辑对象 */
