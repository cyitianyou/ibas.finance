...
namespace finance {
    export namespace app {
        ...
        export class ProjectEditApp extends ibas.BOEditApplication<IProjectEditView, bo.Project> {
            ...
            /** 应用名称 */
            static APPLICATION_NAME: string = "finance_app_project_edit";
            /** 业务对象编码 */
            ...
            run(): void {
                ...
                if (ibas.objects.instanceOf(arguments[0], bo.Project)) {
                    let data: bo.Project = arguments[0];
                    // 新对象直接编辑
                    if (data.isNew) {
                        that.editData = data;
                        that.show();
                    ...
                    if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                        ...
                        boRepository.fetchProject({
                            ...
                            onCompleted(opRslt: ibas.IOperationResult<bo.Project>): void {
                                ...
                                if (ibas.objects.instanceOf(data, bo.Project)) {
                                    // 查询到了有效数据
                                    that.editData = data;
                                    that.show();
                                } else {
            ...
            private chooseOrganization(): void {
                ...
                ibas.servicesManager.runChooseService<initialfantasy.bo.IOrganization>({
                    ...
                    onCompleted(selecteds: ibas.IList<initialfantasy.bo.IOrganization>): void {
                        that.editData.organization = selecteds.firstOrDefault().code;
                    }
                });
            }
            /** 选择项目经理 */
            private chooseManager(): void {
                let that: this = this;
        ...
        /** 视图-项目 */
        export interface IProjectEditView extends ibas.IBOEditView {
            /** 显示数据 */
